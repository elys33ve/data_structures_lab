...
int main(){
	...
	for (div = 0; div < NUM_DIVS; div++)
	{
		cout << div + 1 << "\t";				// show correct Div in final output (fixed from NUM_DIVS + 1)
		for (qtr = 0; qtr < NUM_QTRS; qtr++)
		{
			cout << "$" << sales[div][qtr] << "\t";		// output sales from array (orignally div and qtr were switched)
            totalSales += sales[div][qtr];				// get the sum of all sales (added)
		}
		cout << endl; // Print blank line.
	}
